...
bool est_sup(Record enr1, Record enr2)
{
	...
	if (_strcmpi(enr1.nom, enr2.nom) == 0 && _strcmpi(enr1.prenom, enr2.prenom) > 0) {  //Si noms égaux et prenom mal rangés
	  //Si prenoms mal rangés
		return(true);
	}
	else if (_strcmpi(enr1.nom, enr2.nom) > 0) { //Si noms mal rangés
		return(true);
	}
	else {   //Sinon
		return(false);
